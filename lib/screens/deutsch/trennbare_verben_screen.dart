...
import '../../data/trennbare_verben_questions.dart';
import '../../data/trennbare_verben_theory.dart';
...
/// Теория + упражнения Trennbare Verben (A1 / Start Deutsch 1).
class TrennbareVerbenScreen extends StatefulWidget {
  const TrennbareVerbenScreen({super.key});

  @override
  State<TrennbareVerbenScreen> createState() => _TrennbareVerbenScreenState();
}

class _TrennbareVerbenScreenState extends State<TrennbareVerbenScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tabs;
  late List<TrennbarVerbFrage> _queue;
  ...
  void initState() {
    ...
    _queue = List<TrennbarVerbFrage>.from(allTrennbarVerbenFragen());
  ...
  TrennbarVerbFrage? get _current => _atEnd ? null : _queue.first;

  String _previewLine(TrennbarVerbFrage q) {
  ...
  void _select(int i) {
    ...
    recordGrammarAnswer(
      module: GrammarStatsModule.trennbareVerben,
      ok: ok,
      options: q.options,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Trennbare Verben — A1'),
  ...
  Widget _buildTheoryTab(BuildContext context) {
    return TheoryTabContent(
      headline: 'Trennbare Verben',
      intro: context.s.trennbarTheoryIntro,
      sections: kTrennbarVerbTheorySections,
    );
  }
              ...
              Chip(
                label: Text(
                  context.s.chipProgress(
                    trennbarVerbTeilLabelDe(q.teil),
                    q.nr,
                    _zielAnzahl,
                    _queue.length,
                  ),
