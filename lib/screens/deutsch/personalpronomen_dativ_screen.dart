...
/// Теория + упражнения Personalpronomen im Dativ (A1 / Start Deutsch 1).
class PersonalpronomenDativScreen extends StatefulWidget {
  const PersonalpronomenDativScreen({super.key});

  @override
  State<PersonalpronomenDativScreen> createState() =>
      _PersonalpronomenDativScreenState();
}

class _PersonalpronomenDativScreenState extends State<PersonalpronomenDativScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tabs;
  late List<PersonalpronomenDatFrage> _queue;
  ...
  void initState() {
    ...
    _queue = List<PersonalpronomenDatFrage>.from(allPersonalpronomenDatFragen());
  ...
  PersonalpronomenDatFrage? get _current => _atEnd ? null : _queue.first;

  String _previewLine(PersonalpronomenDatFrage q) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Personalpronomen — Dativ — A1'),
  ...
  Widget _buildTheoryTab(BuildContext context) {
    return TheoryTabContent(
      headline: 'Personalpronomen im Dativ',
      intro: context.s.persDatTheoryIntro,
      sections: kPersonalpronomenDatTheorySections,
    );
  }
              ...
              Chip(
                label: Text(
                  context.s.chipProgress(
                    personalpronomenDatTeilLabelDe(q.teil),
                    q.nr,
                    _zielAnzahl,
                    _queue.length,
                  ),
