...
import '../../data/possessivartikel_akkusativ_questions.dart';
import '../../data/possessivartikel_akkusativ_theory.dart';
...
/// Теория + упражнения Possessivartikel im Akkusativ (A1 / Start Deutsch 1).
class PossessivartikelAkkusativScreen extends StatefulWidget {
  const PossessivartikelAkkusativScreen({super.key});

  @override
  State<PossessivartikelAkkusativScreen> createState() =>
      _PossessivartikelAkkusativScreenState();
}

class _PossessivartikelAkkusativScreenState
    extends State<PossessivartikelAkkusativScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tabs;
  late List<PossessivartikelAkkFrage> _queue;
  ...
  void initState() {
    ...
    _queue = List<PossessivartikelAkkFrage>.from(allPossessivartikelAkkFragen());
  ...
  PossessivartikelAkkFrage? get _current => _atEnd ? null : _queue.first;

  String _previewLine(PossessivartikelAkkFrage q) {
  ...
  void _select(int i) {
    ...
    recordGrammarAnswer(
      module: GrammarStatsModule.possessivartikelAkkusativ,
      ok: ok,
      options: q.options,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Possessivartikel — Akkusativ — A1'),
  ...
  Widget _buildTheoryTab(BuildContext context) {
    return TheoryTabContent(
      headline: 'Possessivartikel im Akkusativ',
      intro: context.s.possAkkTheoryIntro,
      sections: kPossessivartikelAkkTheorySections,
    );
  }
              ...
              Chip(
                label: Text(
                  context.s.chipProgress(
                    possessivartikelAkkTeilLabelDe(q.teil),
                    q.nr,
                    _zielAnzahl,
                    _queue.length,
                  ),
